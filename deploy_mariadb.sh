...
# This script automates the deployment of a MariaDB instance on a Kubernetes cluster.
...
# Function to generate a random password for MariaDB.
generate_password() {
  tr -dc A-Za-z0-9 </dev/urandom | head -c 16
}

# Prompt the user for MariaDB credentials, with the option to auto-generate passwords.
while [ -z "$root_pass" ]; do
  read -p "Enter MariaDB root password (press Enter to generate one): " root_pass
  if [ -z "$root_pass" ]; then
...
while [ -z "$app_user" ]; do
  read -p "Enter MariaDB app user: " app_user
done

while [ -z "$app_db" ]; do
  read -p "Enter MariaDB app database: " app_db
done

while [ -z "$app_pass" ]; do
  read -p "Enter MariaDB app password (press Enter to generate one): " app_pass
  if [ -z "$app_pass" ]; then
